...
import { OctreeNode, computeMapBounds } from 'shared';
...
// === TexturesDictionary Begin ===
const TexturesDictionary = (() => {
    const cache = {};
    const pending = {};
...
// === TexturesDictionary End ===

// === ModelsDictionary Begin ===
        ...
        const promise = new Promise((resolve, reject) => {
            loader.load(
                ...
                (gltf) => {
                    ...
                    // Wrap in a group to isolate and allow centering
                    const wrapper = new THREE.Group();
                    wrapper.name = `Wrapped_${modelName}`;
                    wrapper.add(found);

                    // Compute box before adjusting
                    const box = new THREE.Box3().setFromObject(wrapper);
                    const center = new THREE.Vector3();
                    box.getCenter(center);

                    // Shift children to be centered
                    wrapper.children.forEach(child => {
                        child.position.sub(center);
                    });

                    // Compute final bounding box/size after shift
...
const MapSystem = (() => {
    ...
    async function loadMap(rawMapData) {
        console.log("🧭 loadMap starting", Date.now());
        clearMap();
        mapLoaded = false;

        const mapData = JSON.parse(JSON.stringify(rawMapData));
        ...
        mapLoaded = true;
        console.log("✅ mapLoaded:", mapLoaded, "| objects:", mapObjects.length);
    }
    ...
    function loadHealthPacks(healthPacksData) {
        healthPacksData.forEach((data, i) => {
            const mesh = createHealthPackMesh();
            mesh.position.set(data.position.x, data.position.y, data.position.z);
            mesh.userData.id = data.id;
            mesh.userData.active = data.available;
            scene.add(mesh);
    ...
    function finalizeWorldBoxes(objects) {
        for (const mesh of objects) {
            mesh.updateMatrixWorld(true);
            const worldBox = new THREE.Box3().setFromObject(mesh);
            mesh.userData.box = worldBox;
        }
    }

    function buildOctree(objects) {
        const bounds = computeMapBounds(objects);
        console.log("🧱 Octree bounds:", bounds);
    ...
    function createHealthPackMesh() {
        ...
        const box1 = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.2, 0.2), material);
        const box2 = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.6, 0.2), material);
    ...
    function healthPackTaken({ id, targetPlayerId, health }) {
        const pack = getHealthPacks().find(p => p.userData.id === id);
        if (pack) {
